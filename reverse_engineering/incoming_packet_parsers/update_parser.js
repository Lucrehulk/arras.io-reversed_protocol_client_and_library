class update_parser {
    ...
    parse(packet, offsets, encoded_packet) {
        ...
        for (let entity in this.entities) {
            let dx_check = Math.abs(this.entities[entity].x - this.player.x) > this.player.fov + 60 + this.entities[entity].size * 2;
            let dy_check = Math.abs(this.entities[entity].y - this.player.y) > this.player.fov * 0.5 + 60 + this.entities[entity].size * 2;
            if (this.tick - this.entities[entity].last_updated > 5 && this.entities[entity].layer !== 11 && (
                dx_check ||
                dy_check ||
